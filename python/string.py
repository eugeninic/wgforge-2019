...
def donuts(count):
    return "Количество пончиков: {}".format(count if count < 10 else 'много')
...
# Этот ужин хорош!
def not_bad(s):
    not_i, bad_i = s.find('не'), s.find('плох')
    return s[:not_i] + 'хорош' + s[bad_i + len('плох'):] if -1 < not_i < bad_i else s  
#     return re.sub(r'не(.*)?плох', 'хорош', s) # better
...
def front_back(a, b):
    ...
    strings = (a, b)
    res = ''
     # можно разбить на большее количество частей и последовательно склеить или изменить количество строк и всё будет работать
    for part in zip(*map(half_split, strings)):
        res += ''.join(part)
    return res
